...
| "is_value (VarExp x v) = (v \<noteq> NoRef)"
...
| "is_sexp (VarExp x v) = (v \<noteq> NoRef)"
...
fun deref_vars where
  "deref_vars x NoRef = {}"
| "deref_vars x SelfRef = {x}"  
| "deref_vars x (OtherRef y) = {x, y}"
  
fun ref_vars :: "p_exp \<Rightarrow> string set" where
  "ref_vars (ConstExp c) = {}"
| "ref_vars (OpExp xop) = {}"
| "ref_vars (VarExp x r) = deref_vars x r"
...
definition non_prim_vars where
  "non_prim_vars env e = { x | x. non_prim_entry env x \<and> x \<in> free_vars e }"    
...
definition pure_fun where
  ...
  (*
definition impure_fun where
  "impure_fun t1 t2 = FunTy t1 t2 UsePerm Aff"  
  
 *)
(*
fun is_unique where
  "is_unique (ArrayTy a) = True"
| "is_unique (PairTy t1 t2) = (is_unique t1 \<or> is_unique t2)"  
| "is_unique (FunTy t1 t2 r a) = (a = Aff)"
| "is_unique other = False"  

definition take_fun where
  "take_fun t1 t2 a = (if is_unique t1 then FunTy t1 t2 OwnPerm a else FunTy t1 t2 UsePerm a)" 
  *)
...
(*| "const_type NewArrayConst = {pure_fun (PairTy IntTy (pure_fun IntTy t)) (ArrayTy t) | t. unlim t}"*)
...
| "const_type WriteConst = {pure_fun (ArrayTy t) (FunTy (PairTy IntTy t OwnPerm) UnitTy OwnPerm Ref) Prim | t. unlim t}"
  ...
  *)
(*| "const_type PairConst = {FunTy t1 (FunTy t2 (PairTy t1 t2 r) r Ref) r Prim | t1 t2 r. True}"*)
  (*
...
fun var_val_type :: "var_val_ref \<Rightarrow> p_type \<Rightarrow> p_type \<Rightarrow> bool" where
  "var_val_type NoRef tau tau_x = True"
| "var_val_type r tau (ArrayTy tau_x) = (req_type tau \<noteq> Prim)"
| "var_val_type r tau (ChanTy tau_x c_end) = (req_type tau \<noteq> Prim)"
| "var_val_type r tau tau_x = False" 
...
definition deref_name where
  "deref_name x r = (case r of
    NoRef \<Rightarrow> x
    | SelfRef \<Rightarrow> x
    | OtherRef a \<Rightarrow> a
  )"
...
| "well_typed env r_s1 (OpExp xop) tau r_s2 rx = (tau = op_type xop \<and> leq_use_env r_s2 r_s1 \<and> leq_use_env rx r_s2)"
    (* in a variable value, the name carries the owner, but the actual type must be dereferenced *)
| "well_typed env r_s1 (VarExp x v) tau r_s2 rx = (\<exists> r_ex tau_x. env (deref_name x v) = Some tau \<and> env x = Some tau_x \<and> var_val_type v tau tau_x \<and>
    leq_use_env (ereq_use_env x tau_x) r_s1 \<and> leq_use_env r_s2 (diff_use_env r_s1 (comp_use_env (ereq_use_env x tau_x) r_ex)) \<and>
    leq_use_env rx r_s2 \<and> leq_use_env r_ex r_s1 \<and> leq_use_env (diff_use_env (ereq_use_env x tau_x) (comp_use_env (ereq_use_env x tau_x) r_ex)) rx)"
| "well_typed env r_s1 (PairExp e1 e2) tau r_sf rf = (\<exists> t1 t2 r r_s2 r_s3 rx1 rx2 r_ex. tau = PairTy t1 t2 r \<and>
    well_typed env r_s1 e1 t1 r_s2 rx1 \<and> well_typed env r_s2 e2 t2 r_s3 rx2 \<and> (*aff_use_env rx (req_type tau) \<and>*)
    (*safe_pair_aff (max_aff (req_type t1) (req_type t2)) r \<and>*)
    ...
    (*safe_use_lift rx1 r \<and> safe_use_lift rx2 r \<and> safe_type t1 r \<and> safe_type t2 r
    (*leq_use_env rx1 rx \<and> leq_use_env rx2 rx*) \<and>*) disj_use_env (lift_use_env rx1 r) (lift_use_env rx2 r) \<and>
...
(*| "well_typed env r_s1 (PairExp e1 e2) tau r_s2 rf = (\<exists> t1 t2 r rx rx1 rx2 r_xa r_xb r_sa r_sb r_ex. tau = PairTy t1 t2 r \<and>
    well_typed env rx1 e1 t1 r_sa r_xa \<and> well_typed env rx2 e2 t2 r_sb r_xb \<and> (*aff_use_env rx (req_type tau) \<and>*) r \<noteq> NoPerm \<and> leq_use_env rx r_s1 \<and>
    leq_use_env (lift_use_env rx1 r) rx \<and> leq_use_env (lift_use_env rx2 r) rx
    (*leq_use_env rx1 rx \<and> leq_use_env rx2 rx*) \<and> disj_use_env (lift_use_env rx1 r) (lift_use_env rx2 r) \<and>
    leq_use_env r_s2 (diff_use_env r_s1 r_ex) \<and> leq_use_env rf r_s2 \<and> leq_use_env r_ex r_s1 \<and> leq_use_env (pair_req rx r_ex tau) rf
  )"*)
...
| "well_typed env r_s1 (LamExp x e) tau r_s2 rf = (\<exists> t1 t2 r a rx r_end r_s' r_ex. tau = FunTy t1 t2 r a \<and> x \<notin> ref_vars e \<and>
...
| "well_typed env r_s1 (AppExp e1 e2) tau r_sf rx = (\<exists> t1 r a r_s2 rx1 rx2 r_s3 r_ex.
    ...
    leq_use_env r_sf (diff_use_env r_s3 (comp_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_ex)) \<and> (*r \<noteq> NoPerm \<and>*) (*safe_use_lift rx2 r \<and>
  ...
  (* let's briefly explain the permissions system. all expressions are typed by the permissions available,
    as well as the permissions required to further evaluate the expression.
      - var case. requires x as a use-permission, and also requires ownership of the x. it is not
          strictly necessary to have this condition, however doing so removes many edge cases.
      - lam case. an arbitrary set of permissions are pulled from the start and are allowed in the
          lambda. these permissions are then given as the evaluation requirements.
          note: requirement permissions may be dropped if they are also dropped from the available permissions.
      - app case. the requisite permissions are removed from the environment, with the remaining permissions
          being any uses from either expression. function types are decorated with a requirement for the
          argument type which can "upgrade" the permissions required. for instance, a mere use can be
          upgraded into ownership.

      - requirements can be dropped for any s-exp. for const/op it's because they dont have any.
        for var, it's because only one permission is relevant. for lambda it's by construction.
        for the fix-point case, it's bc the rhs is a lambda.
      - we also want to allow permissions to be required liberally (for the sake of substitution).
        for const/op we can allow non-empty requirements. for var, we minimally specify rx. for
        lambda it's already possible since rx is arbitrary. for if/app, one can simply extend e2.

    the other issue is the idea of an affine function. ie, we want to make sure no "use"
    functions are secretly hiding ownership values. so we designate them as affine if they are.
  *)      
