...
| "subst_exp (VarExp x' v) x e = (if x = x' then e else VarExp x' v)"
...
| "deep_alpha_rename (VarExp x' v) x y = (if x = x' then VarExp y v else VarExp x' v)"
...
| "lam_var_remove (VarExp x' v) x y = VarExp x' v"
...
lemma subst_type_preserve_no_fv: "\<lbrakk> well_typed env r_s1 e tau r_s2 rx; x \<notin> free_vars e \<rbrakk> \<Longrightarrow> well_typed env r_s1 (subst_exp e x e') tau r_s2 rx"
  apply (induction e arbitrary: env r_s1 tau r_s2 rx)
    ...
    (* if case *)
     ...
     apply (auto)
    (* lambda case *)
    apply (cut_tac y="x1a" and e="e" and x="x" and e'="e'" in no_subst_ref_vars)
    apply (auto)
