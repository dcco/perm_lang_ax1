...
definition sub_env :: "'a gen_env \<Rightarrow> 'b gen_env \<Rightarrow> bool" where
  "sub_env s env = (\<forall> x. s x = None \<longrightarrow> env x = None)"  
  
definition sub_use_env :: "'a gen_env \<Rightarrow> perm_use_env \<Rightarrow> bool" where
  "sub_use_env s r_s = (\<forall> x. s x = None \<longrightarrow> r_s x = NoPerm)"

definition contain_env :: "'a gen_env \<Rightarrow> 'a gen_env \<Rightarrow> bool" where
...
lemma dist_add_sub_env: "\<lbrakk> sub_env s env \<rbrakk> \<Longrightarrow> sub_env (add_env s x v) (add_env env x tau)"    
  apply (simp add: sub_env_def)
  apply (simp add: add_env_def)
  done  

lemma rhs_add_sub_env: "\<lbrakk> sub_env s env; s x \<noteq> None \<rbrakk> \<Longrightarrow> sub_env s (add_env env x tau)"        
  apply (simp add: sub_env_def)
  apply (simp add: add_env_def)
  done  
...
lemma add_rem_sub_env: "\<lbrakk> sub_env (add_env s x v) env \<rbrakk> \<Longrightarrow> sub_env s (rem_env env x)"    
  apply (simp add: sub_env_def)
  apply (simp add: add_env_def)
  apply (simp add: rem_env_def)
  done
...
lemma rhs_add_sub_use_env: "\<lbrakk> sub_use_env s r_s; s x \<noteq> None \<rbrakk> \<Longrightarrow> sub_use_env s (add_use_env r_s x r)"    
  apply (simp add: sub_use_env_def)
...
lemma add_rem_sub_use_env: "\<lbrakk> sub_use_env (add_env s x v) r_s \<rbrakk> \<Longrightarrow> sub_use_env s (rem_use_env r_s x)"    
  apply (simp add: sub_use_env_def)
  apply (simp add: add_env_def)
  apply (simp add: rem_use_env_def)
  done
