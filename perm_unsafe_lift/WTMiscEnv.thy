...
definition non_prim_entry where
  ...
  (*
definition prim_use_env where
  "prim_use_env env r_s = (\<forall> x. r_s x \<noteq> NoPerm \<longrightarrow> (case env x of
    None \<Rightarrow> False | Some tau \<Rightarrow> req_type tau = Prim))"
  *)
...
definition aff_use_env where
  ...
  (*
definition aff_fun_ty where
  "aff_fun_ty tau = (\<not> unlim tau \<and> fun_ty tau)"   *)
