...
lemma wt_sexp_no_npv_use: "\<lbrakk> well_typed env r_s1 e tau r_s2 rx; is_sexp e; req_type tau = Prim \<rbrakk> \<Longrightarrow> x \<notin> non_prim_vars env e"    
  apply (induct e arbitrary: env r_s1 tau r_s2 rx)
   ...
   apply (auto)
   apply (cut_tac env="add_env env x1a t1" and ?r_s1.0="add_use_env rxa x1a r" and e="e" and x="x" in well_typed_no_npv_use)
     apply (auto)
  ...
  apply (simp add: aff_use_env_def)
  apply (simp add: null_use_env_def)
  apply (cut_tac env="add_env env x61 t" and ?r_s1.0="add_use_env rxa x61 UsePerm" and e="x62" and x="x" in well_typed_no_npv_use)
    apply (auto)
   apply (simp add: add_use_env_def)
  apply (simp add: non_prim_vars_def)
  apply (simp add: non_prim_entry_def)
...
lemma subst_type_preserve_prim_fv: "\<lbrakk> well_typed (add_env env x t) r_s1 e tau r_se r_xe;
  well_typed env rx2 e' t rx2 rx2; is_sexp e'; req_type t = Prim; lam_vars e \<inter> (free_vars e' \<union> ref_vars e') = {}; x \<notin> ref_vars e \<rbrakk> \<Longrightarrow>
  ...
  apply (induct e arbitrary: env tau r_s1 r_se r_xe)
    ...
    (* var case *)
          ...
          apply (rule_tac rx="empty_use_env" in well_typed_incr_req)
            apply (rule_tac ?r_s1.0="r_se" in well_typed_incr_start_perm)
             ...
             apply (simp add: add_env_def)
            apply (simp add: add_env_def)
           apply (rule_tac r_sb="diff_use_env r_s1 (comp_use_env (ereq_use_env x tau_x) r_ex)" in trans_leq_use_env)
            apply (rule_tac self_diff_leq_use_env)
           apply (simp)
          apply (rule_tac leq_empty_use_env)
    (* no-replacement case. *)
         apply (case_tac "deref_name x1a x2a = x")
          apply (simp add: deref_name_def)
          apply (case_tac x2a)
            apply (auto)
         apply (simp add: add_env_def)
    ...
    (* pair case *)
       ...
       apply (auto)
        apply (case_tac "\<not> lam_vars e1 \<inter> (free_vars e' \<union> ref_vars e') = {}")
         apply (auto)
       apply (rule_tac x="rx2a" in exI)
       apply (auto)
       apply (case_tac "\<not> lam_vars e2 \<inter> (free_vars e' \<union> ref_vars e') = {}")
        apply (auto)
    (* if case *)
      ...
      apply (rule_tac x="rx2a" in exI)
      apply (auto)
      apply (case_tac "\<not> lam_vars e3 \<inter> (free_vars e' \<union> ref_vars e') = {}")
       apply (auto)
    (* lam case p1. *)
     apply (rule_tac x="rx" in exI)
     apply (auto)
     ...
     apply (auto)
    apply (cut_tac y="x1a" and e="e" and e'="e'" and x="x" in subst_ref_vars)
      apply (auto)
    (* lam case p2. *)
   apply (rule_tac x="rx" in exI)
   apply (auto)
...
(*
lemma dist_lift_leq_use_env_ex: "\<lbrakk> leq_perm (lift_use_env r_s q x) r \<rbrakk> \<Longrightarrow> leq_use_env (lift_use_env r_s q) (lift_use_env r_s r)"    
  apply (case_tac "lift_use_env r_s q \<noteq> r_s"
   apply (case_tac q)
     apply (auto)
...
lemma stpx_var_case: "
      \<lbrakk>well_typed env rx2 e' t rx2 rx2; rx1 x1a = NoPerm; x1a \<notin> deref_vars x1a x2a; is_sexp e'; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1; (*safe_use_lift rx2 r;*)
        disj_use_env rx1 (lift_use_env rx2 r); add_env env x1a t (deref_name x1a x2a) = Some tau; add_env env x1a t x1a = Some tau_x;
        leq_use_env (ereq_use_env x1a tau_x) (add_use_env rx1 x1a r);
        leq_use_env r_se (diff_use_env (add_use_env rx1 x1a r) (comp_use_env (ereq_use_env x1a tau_x) r_ex)); leq_use_env r_xe r_se;
        leq_use_env r_ex (add_use_env rx1 x1a r); leq_use_env (diff_use_env (ereq_use_env x1a tau_x) (comp_use_env (ereq_use_env x1a tau_x) r_ex)) r_xe\<rbrakk>
    ...
    (* - tau = tau_x. since x is not  *)
  apply (case_tac "\<not> tau_x = tau")
   apply (case_tac "deref_name x1a x2a \<noteq> x1a")
    apply (simp add: deref_name_def)
    apply (case_tac x2a)
      apply (auto)
    (* - r_xe x1a \<le> r *)
  apply (case_tac "\<not> leq_perm (r_xe x1a) r")
   apply (case_tac "\<not> (\<forall> x'. leq_perm (r_xe x') (add_use_env rx1 x1a r x'))")
    apply (cut_tac r_sc="r_xe" and r_sb="diff_use_env (add_use_env rx1 x1a r) (comp_use_env (ereq_use_env x1a tau) r_ex)" and r_sa="add_use_env rx1 x1a r" in trans_leq_use_env)
      apply (rule_tac self_diff_leq_use_env)
  ...
  apply (rule_tac rx="diff_use_env (comp_use_env (rem_use_env r_xe x1a) (stpx_ex rx2 (end_req_perm tau))) (end_neg_use_env r_se x1a rx2 r)" in well_typed_incr_req)
    ...
    (* - proving disjointness of r_se - x / rx2 (required to remove rse from end perms) [comp_perms2] *)
       ...
       apply (rule_tac r_sb="diff_use_env (add_use_env rx1 x1a r) (comp_use_env (ereq_use_env x1a tau) r_ex)" in trans_leq_use_env)
        apply (rule_tac self_diff_leq_use_env)
       apply (simp)
   ...
   apply (case_tac "is_own (r_xe x1a)")
    apply (rule_tac dist_diff_leq_use_env)
    apply (case_tac "stpx_req (rem_use_env r_xe x1a) (lift_use_env rx2 (r_xe x1a)) (r_xe x1a) \<noteq>
      comp_use_env (rem_use_env r_xe x1a) (lift_use_env rx2 (r_xe x1a))")
     apply (simp add: is_own_def)
     apply (simp add: stpx_req_def)
    apply (auto)
    apply (rule_tac dist_comp_leq_use_env)
   ...
   apply (case_tac "r_xe x1a")
    ...
    (* - otherwise, ereq_use_env x1a tau x1a \<noteq> None *)
     apply (case_tac "ereq_use_env x1a tau x1a = NoPerm")
    ...
    (* - knowing this, (ereq x1a tau + r_ex) x1a = Own, since r_xe = None *)
     apply (cut_tac r_x="ereq_use_env x1a tau" and r_ex="comp_use_env (ereq_use_env x1a tau) r_ex" and r_s="r_xe" and x="x1a" in diff_use_own)
        apply (auto)
    (* - this means r = OwnPerm, since (ereq x1a tau + r_ex) \<le> rx1 + (x: r) *)
     apply (case_tac "\<not> is_own r")
      apply (cut_tac r_x="comp_use_env (ereq_use_env x1a tau) r_ex" and r_s="add_use_env rx1 x1a r" and x="x1a" in leq_use_own)
        apply (simp)
       apply (rule_tac dist_comp_leq_use_env)
        apply (auto)
      apply (simp add: add_use_env_def)
    ...
    (* - we also have r_se x = None *)
     apply (cut_tac r_x="r_se" and r_s="diff_use_env (add_use_env rx1 x1a r) (comp_use_env (ereq_use_env x1a tau) r_ex)" and x="x1a" in leq_use_none)
       apply (simp)
    ...
    apply (case_tac "req_type tau = Aff")
     apply (cut_tac r_x="r_xe" and r_s="diff_use_env (add_use_env rx1 x1a r) (comp_use_env (ereq_use_env x1a tau) r_ex)" and x="x1a" in leq_use_none)
       apply (rule_tac r_sb="r_se" in trans_leq_use_env)
        apply (auto)
     apply (rule_tac diff_use_none_ex)
     apply (case_tac "comp_use_env (ereq_use_env x1a tau) r_ex x1a \<noteq> OwnPerm")
      apply (cut_tac r_sa="ereq_use_env x1a tau" and r_sb="r_ex" and x="x1a" in comp_use_no_own_both)
       apply (auto)
     apply (simp add: ereq_use_env_def)
     apply (simp add: one_use_env_def)
     apply (simp add: end_req_perm_def)
...
lemma dist_stpx_req_leq_use_env: "\<lbrakk> leq_perm q r; leq_use_env r_x r_s; leq_use_env r_exb r_exa \<rbrakk> \<Longrightarrow>
  ...
  apply (rule_tac dist_stpx_ex_leq_use_env_gen)
   apply (simp_all)
    ...
    (*disj_use_env (stpx_req (rem_use_env r_xa x) (lift_use_env rx2 (r_xa x)) (r_xa x)) (end_neg_use_env r_sa x rx2 r)
    r = r
    r_sa x = r_x x
    r_xa x = q
*)
lemma disj_stpx_end_neg_use_env: "\<lbrakk> leq_perm q (r_x x); leq_perm (r_x x) r; disj_use_env r_ex (lift_use_env r_s r) \<rbrakk> \<Longrightarrow>
  ...
  apply (case_tac "r_x x")
    ...
    (*
    \<Longrightarrow> leq_use_env (end_neg_use_env r_sa x rx2 r) (stpx_req (rem_use_env rx1a x) (lift_use_env rx2 r) (rx1a x)) *)(*
    thm end_neg_use_env_def
lemma end_neg_stpx_leq_use_env: "\<lbrakk> leq_perm (r_x x) r \<rbrakk> \<Longrightarrow> leq_use_env (end_neg_use_env r_x x r_s q) (stpx_req r_ex (lift_use_env r_s q) r)"    
  apply (simp add: end_neg_use_env_def)
  apply (auto)
   apply (simp add: stpx_req_def)
    apply (auto)*)
...
lemma well_typed_stpx_req2:
  ...
  apply (rule_tac dist_stpx_req_leq_use_env)
   ...
   apply (auto)
    ...
    (*
lemma stpx_req_use_env: "\<lbrakk> q \<noteq> NoPerm; leq_perm q r; safe_use_lift r_s r \<rbrakk> \<Longrightarrow> stpx_req r_x (lift_use_env r_s r) q = comp_use_env r_x (lift_use_env r_s r)"    
  apply (case_tac q)
    apply (auto)
   apply (simp add: stpx_req_def)*)
    
    
lemma subst_type_preserve_no_npv: "\<lbrakk> well_typed (add_env env x t) r_s1 e tau r_se r_xe;
  well_typed env rx2 e' t rx2 rx2; is_sexp e'; x \<notin> non_prim_vars (add_env env x t) e;
  lam_vars e \<inter> (free_vars e' \<union> ref_vars e') = {}; x \<notin> ref_vars e \<rbrakk> \<Longrightarrow>
  well_typed env r_s1 (subst_exp e x e') tau r_se r_xe"  
  apply (simp add: non_prim_vars_def)
  apply (auto)
   apply (simp add: non_prim_entry_def)
...
lemma dist_drop_leq_use_env: "\<lbrakk> leq_perm q r \<rbrakk> \<Longrightarrow> leq_use_env (drop_use_env_ex r_s q) (drop_use_env_ex r_s r)"
...
lemma stpx_req_leq_use_env5: "\<lbrakk> leq_use_env (comp_use_env r_x (lift_use_env (drop_use_env_ex r_s r) r)) r_ex \<rbrakk> \<Longrightarrow>
  ...
  apply (rule_tac id_leq_use_env)
   ...
   (*
lemma self_stpx_req_drop_leq_use_env: "\<lbrakk> q \<noteq> NoPerm; leq_perm q r; safe_use_lift r_s r \<rbrakk> \<Longrightarrow>
  leq_use_env (comp_use_env r_x (lift_use_env (drop_use_env_ex r_s q) r)) (stpx_req r_x (lift_use_env r_s r) q)"
...
(*
lemma stpxpc_leq_use_env1: "\<lbrakk> leq_use_env rx1a rx; ra \<noteq> NoPerm; leq_perm (rx1a x) (rx x)\<rbrakk>
...
(*
lemma stpxpc_leq_use_env2: "\<lbrakk> leq_use_env rx1a rx; ra \<noteq> NoPerm; leq_perm (rx1a x) (rx x) \<rbrakk> \<Longrightarrow> 
  leq_use_env (stpx_req (rem_use_env rx1a x) (lift_use_env rx2 (rx1a x)) (rx1a x))
...
lemma stpxpc_disj_use_env1: "\<lbrakk> disj_use_env (lift_use_env rx1a ra) (lift_use_env rx2a ra); disj_use_env rx1 (lift_use_env rx2 r);
  leq_perm (rx2a x) r; ra \<noteq> NoPerm; safe_use_lift rx2 r; leq_use_env rx1a rx;
  leq_use_env rx2a rx; leq_use_env rx (add_use_env rx1 x r) \<rbrakk> \<Longrightarrow>
  disj_use_env (lift_use_env (rem_use_env rx1a x) ra) (lift_use_env (stpx_ex (lift_use_env rx2 (rx2a x)) (rx2a x)) ra)"
    (* we want to show that lifted rx1a is disjoint from twice-lifted rx2. if ra = Use, this is easy.
        if ra = Own, both sides will be maximally lifted. the idea here is that in such a case, since
     *)
...
lemma wtapr_helper: "\<lbrakk>\<And>env r_s1 tau r_s2 rx.
           \<lbrakk>well_typed env (add_use_env r_s1 x r) e tau r_s2 rx; x \<notin> free_vars e\<rbrakk> \<Longrightarrow> well_typed env r_s1 e tau (rem_use_env r_s2 x) (rem_use_env rx x);
        well_typed (add_env env x t1) (add_use_env rxa x ra) e t2 r_s' r_end; aff_use_env rxa a; leq_use_env rxa (add_use_env r_s1 x r);
        leq_use_env r_s2 (diff_use_env (add_use_env r_s1 x r) r_ex); leq_use_env rx r_s2; leq_use_env r_ex (add_use_env r_s1 x r);
        leq_use_env (diff_use_env rxa r_ex) rx\<rbrakk>
       \<Longrightarrow> \<exists>rxa. (\<exists>r_end r_s'. well_typed (add_env env x t1) (add_use_env rxa x ra) e t2 r_s' r_end) \<and>
                 aff_use_env rxa a \<and>
                 leq_use_env rxa r_s1 \<and>
                 (\<exists>r_ex. leq_use_env (rem_use_env r_s2 x) (diff_use_env r_s1 r_ex) \<and>
                         leq_use_env (rem_use_env rx x) (rem_use_env r_s2 x) \<and> leq_use_env r_ex r_s1 \<and> leq_use_env (diff_use_env rxa r_ex) (rem_use_env rx x))"
  apply (rule_tac x="rem_use_env rxa x" in exI)
  apply (auto)
     apply (rule_tac t="add_use_env (rem_use_env rxa x) x ra" and s="add_use_env rxa x ra" in subst)
...
lemma well_typed_add_perms_rev: "\<lbrakk> well_typed env (add_use_env r_s1 x r) e tau r_s2 rx; x \<notin> free_vars e \<rbrakk> \<Longrightarrow>
  ...
  apply (induct e arbitrary: env r_s1 tau r_s2 rx)
    ...
    (* const + op cases *)
         ...
         apply (auto)
    (* var cases *)
        apply (rule_tac r_sb="rem_use_env (ereq_use_env x1a tau_x) x" in trans_leq_use_env)
         apply (rule_tac r="r" in rem_add_leq_use_env)
         apply (auto)
       ...
       apply (auto)
          apply (rule_tac r="r" in rem_add_leq_use_env)
          apply (rule_tac r_sb="add_use_env (diff_use_env r_s1 (comp_use_env (ereq_use_env x1a tau_x) r_ex)) x r" in trans_leq_use_env)
           apply (rule_tac dist_add_leq_use_env)
           apply (rule_tac dist_diff_leq_use_env_gen)
            apply (rule_tac id_leq_use_env)
           apply (rule_tac dist_comp_leq_use_env)
           ...
           apply (rule_tac comp_leq_use_env2)
           apply (rule_tac self_rem_leq_use_env)
          apply (rule_tac r_sb="diff_use_env (add_use_env r_s1 x r) (comp_use_env (ereq_use_env x1a tau_x) r_ex)" in trans_leq_use_env)
           apply (rule_tac add_diff_leq_use_env)
          apply (simp)
         apply (rule_tac dist_rem_leq_use_env)
         apply (simp)
        apply (rule_tac r="r" in rem_add_leq_use_env)
        apply (auto)
       apply (rule_tac r_sb="rem_use_env (diff_use_env (ereq_use_env x1a tau_x) (comp_use_env (ereq_use_env x1a tau_x) r_ex)) x" in trans_leq_use_env)
        apply (rule_tac dist_rem_leq_use_env)
        apply (simp)
    ...
    apply (case_tac "x \<noteq> x1a")
     ...
     apply (rule_tac x="rem_use_env rxa x" in exI)
     apply (auto)
        apply (rule_tac t="add_use_env (rem_use_env rxa x) x1a ra" and s="rem_use_env (add_use_env rxa x1a ra) x" in subst)
...
lemma subst_np_vars: "\<lbrakk> x \<in> non_prim_vars env (subst_exp e y e') \<rbrakk> \<Longrightarrow> (x \<in> non_prim_vars env e \<and> x \<noteq> y) \<or> x \<in> non_prim_vars env e'"    
  apply (simp add: non_prim_vars_def)
  apply (auto)
   apply (cut_tac x="x" and y="y" and e="e" and e'="e'" in subst_free_vars)
    apply (auto)
  apply (cut_tac x="x" and y="y" and e="e" and e'="e'" in subst_free_vars)
   apply (auto)
  done
        
lemma stpxic_valid_diff: "\<lbrakk> xa \<in> non_prim_vars env (subst_exp e2 x e'); xa \<in> own_env_vars (end_neg_use_env r_s2 x rx2 r);
  x \<in> free_vars e2; well_typed env rx2 e' t rx2 rx2; is_sexp e'; disj_use_env rx1 (lift_use_env rx2 r);
  well_typed (add_env env x t) (add_use_env rx1 x r) e1 tau' r_s2 rx'; well_typed (add_env env x t) r_s2 e2 tau r_se rx \<rbrakk> \<Longrightarrow> False"    
    (* if r_s2 x \<noteq> None, end_neg contains no vars. so then r_s2 x = None. since we have x \<in> free_vars e2, t must be Prim *)
  apply (case_tac "r_s2 x \<noteq> NoPerm")
   apply (simp add: own_env_vars_def)
   apply (simp add: end_neg_use_env_def)
   apply (simp add: empty_use_env_def)
  apply (case_tac "req_type t \<noteq> Prim")
   apply (cut_tac env="add_env env x t" and ?r_s1.0="r_s2" and e="e2" and x="x" in well_typed_no_npv_use)
     apply (auto)
   apply (simp add: non_prim_vars_def)
   apply (simp add: non_prim_entry_def)
   apply (simp add: add_env_def)
  ...
  apply (cut_tac e="e2" and e'="e'" and x="xa" and y="x" in subst_np_vars)
   ...
   apply (case_tac "r_s2 xa = NoPerm")
    apply (cut_tac env="add_env env x t" and ?r_s1.0="r_s2" and e="e2" and x="xa" in well_typed_no_npv_use)
      apply (auto)
    apply (simp add: non_prim_vars_def)
    apply (simp add: non_prim_entry_def)
    apply (simp add: add_env_def)
   ...
   apply (case_tac "rx1 xa = NoPerm")
    apply (case_tac "add_use_env rx1 x r xa = NoPerm")
...
lemma stpxpc_coercex: "\<lbrakk>\<And>env x tau t rx1 r_se r_xe r_s1 rx2 r.
           \<lbrakk>well_typed (add_env env x t) (add_use_env rx1 x r) e1 tau r_se r_xe; well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; x \<in> free_vars e1;
            lam_vars e1 \<inter> (free_vars e' \<union> ref_vars e') = {}; x \<notin> ref_vars e1; (*r \<noteq> NoPerm;*) leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1; (*safe_use_lift rx2 r;*)
            disj_use_env rx1 (lift_use_env rx2 r)\<rbrakk>
           \<Longrightarrow> well_typed env r_s1 (subst_exp e1 x e') tau (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r))
                (diff_use_env (stpx_req (rem_use_env r_xe x) (lift_use_env rx2 (r_xe x)) (r_xe x)) (end_neg_use_env r_se x rx2 r));
  well_typed (add_env env x t) (add_use_env rx1 x r) e1 tau r_se r_xe; well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; x \<in> free_vars e1;
         lam_vars e1 \<inter> (free_vars e' \<union> ref_vars e') = {}; x \<notin> ref_vars e1; (*r \<noteq> NoPerm;*) leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1; (*safe_use_lift rx2 r;*) disj_use_env rx1 (lift_use_env rx2 r)
  \<rbrakk> \<Longrightarrow> well_typed env r_s1 (subst_exp e1 x e') tau (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r))
             (diff_use_env (stpx_req (rem_use_env r_xe x) (lift_use_env rx2 (r_xe x)) (r_xe x)) (end_neg_use_env r_se x rx2 r))"    
  apply (auto)
  done
...
lemma stpx_pair_case: "\<lbrakk>\<And>env x tau t rx1 r_se r_xe r_s1 rx2 r.
           \<lbrakk>well_typed (add_env env x t) (add_use_env rx1 x r) e1 tau r_se r_xe; well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; x \<in> free_vars e1;
            x \<notin> ref_vars e1; lam_vars e1 \<inter> (free_vars e' \<union> ref_vars e') = {}; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1;
            disj_use_env rx1 (lift_use_env rx2 r)\<rbrakk>
           \<Longrightarrow> well_typed env r_s1 (subst_exp e1 x e') tau
                (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r))
                (diff_use_env (stpx_req (rem_use_env r_xe x) (lift_use_env rx2 (r_xe x)) (r_xe x)) (end_neg_use_env r_se x rx2 r));
        \<And>env x tau t rx1 r_se r_xe r_s1 rx2 r.
           \<lbrakk>well_typed (add_env env x t) (add_use_env rx1 x r) e2 tau r_se r_xe; well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; x \<in> free_vars e2;
            x \<notin> ref_vars e2; lam_vars e2 \<inter> (free_vars e' \<union> ref_vars e') = {}; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1;
            disj_use_env rx1 (lift_use_env rx2 r)\<rbrakk>
           \<Longrightarrow> well_typed env r_s1 (subst_exp e2 x e') tau
                (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r))
                (diff_use_env (stpx_req (rem_use_env r_xe x) (lift_use_env rx2 (r_xe x)) (r_xe x)) (end_neg_use_env r_se x rx2 r));
        well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; is_sexp e'; (lam_vars e1 \<union> lam_vars e2) \<inter> (free_vars e' \<union> ref_vars e') = {};
        leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1; disj_use_env rx1 (lift_use_env rx2 r); x \<notin> ref_vars e1; x \<notin> ref_vars e2;
        well_typed (add_env env x t) (add_use_env rx1 x r) e1 t1 r_s2 rx1a; well_typed (add_env env x t) r_s2 e2 t2 r_s3 rx2a;
        ...
        leq_use_env r_ex (add_use_env rx1 x r);
  ...
  apply (case_tac "\<not> leq_perm (lift_use_env rx1a ra x) r")
   apply (cut_tac r_x="lift_use_env rx1a ra" and r_s="add_use_env rx1 x r" and x="x" in spec_leq_perm)
    apply (rule_tac r_sb="r_s3" in trans_leq_use_env)
     apply (auto)
   apply (simp add: add_use_env_def)
    (* prelim: lift rx2a ra x \<le> r *)
  apply (case_tac "\<not> leq_perm (lift_use_env rx2a ra x) r")
   ...
   apply (case_tac "x \<notin> free_vars e1")
    apply (rule_tac well_typed_diff_end_perm)
     apply (rule_tac ?r_s1.0="comp_use_env rx1 (lift_use_env rx2 r)" in well_typed_incr_start_perm)
      apply (rule_tac rx="rem_use_env rx1a x" in well_typed_incr_req)
        apply (rule_tac well_typed_comp_perms)
         apply (rule_tac subst_type_preserve_no_fv)
          apply (rule_tac r="r" in well_typed_add_perms_rev)
    ...
    apply (rule_tac self_comp_leq_use_env2)
    (* x \<in> fv e1 *)
   apply (rule_tac env="env" and ?rx1.0="rem_use_env rx1 x" and ?e1.0="e1" and ?r_s1.0="r_s1" and x="x" in stpxpc_coercex)
           apply (auto)
      apply (rule_tac t="add_use_env (rem_use_env rx1 x) x r" and s="add_use_env rx1 x r" in subst)
       apply (rule_tac partial_add_rem_use_env)
      apply (simp)
     apply (simp add: rem_use_env_def)
    apply (rule_tac r_sb="comp_use_env rx1 (lift_use_env rx2 r)" in trans_leq_use_env)
  ...
  apply (auto)
      apply (case_tac "\<not> well_typed (add_env env x t) (add_use_env (rem_use_env r_s2 x) x (r_s2 x)) e2 t2 r_s3 rx2a")
      ...
      apply (case_tac "x \<notin> free_vars e2")
       apply (rule_tac r_s="r_s2" in well_typed_diff_decr_end_neg_perm)
         apply (rule_tac rx="diff_use_env (rem_use_env rx2a x) (end_neg_use_env r_s2 x rx2 r)" in well_typed_incr_req)
           apply (rule_tac t="diff_use_env (comp_use_env (rem_use_env r_s2 x) (lift_use_env rx2 r)) (end_neg_use_env r_s2 x rx2 r)" and
      s="comp_use_env (diff_use_env (rem_use_env r_s2 x) (end_neg_use_env r_s2 x rx2 r)) (diff_use_env (lift_use_env rx2 r) (end_neg_use_env r_s2 x rx2 r))" in subst)
          apply (rule_tac dist_diff_comp_use_env)
           apply (rule_tac t="diff_use_env (comp_use_env (rem_use_env r_s3 x) (lift_use_env rx2 r)) (end_neg_use_env r_s2 x rx2 r)" and
      s="comp_use_env (diff_use_env (rem_use_env r_s3 x) (end_neg_use_env r_s2 x rx2 r)) (diff_use_env (lift_use_env rx2 r) (end_neg_use_env r_s2 x rx2 r))" in subst)
            apply (rule_tac dist_diff_comp_use_env)
           apply (rule_tac well_typed_comp_perms)
    ...
    (* x \<in> fv e2. we start by removing end_neg r_s2 x  *)
      apply (case_tac "\<not> (non_prim_vars env (subst_exp e2 x e') \<inter> own_env_vars (end_neg_use_env r_s2 x rx2 r) = {})")
       apply (auto)
       apply (rule_tac xa="xa" and env="env" and ?e1.0="e1" and ?e2.0="e2" and e'="e'" in stpxic_valid_diff)
              apply (auto)
      apply (rule_tac t="diff_use_env (comp_use_env (rem_use_env r_s3 x) (lift_use_env rx2 r)) (end_neg_use_env r_s3 x rx2 r)" and
        s="diff_use_env (diff_use_env (comp_use_env (rem_use_env r_s3 x) (lift_use_env rx2 r)) (end_neg_use_env r_s3 x rx2 r))
          (end_neg_use_env r_s2 x rx2 r)" in subst)
       apply (cut_tac r_s="comp_use_env (rem_use_env r_s3 x) (lift_use_env rx2 r)" and r_x="end_neg_use_env r_s2 x rx2 r" and
          r_c="end_neg_use_env r_s3 x rx2 r" in cancel_diff_use_env2)
        apply (rule_tac dist_end_neg_leq_use_env)
        apply (rule_tac well_typed_perm_leq)
        apply (auto)
      apply (rule_tac t="diff_use_env (stpx_req (rem_use_env rx2a x) (lift_use_env rx2 (rx2a x)) (rx2a x)) (end_neg_use_env r_s3 x rx2 r)" and
        s="diff_use_env (diff_use_env (stpx_req (rem_use_env rx2a x) (lift_use_env rx2 (rx2a x)) (rx2a x)) (end_neg_use_env r_s3 x rx2 r))
          (end_neg_use_env r_s2 x rx2 r)" in subst)
       apply (cut_tac r_s="stpx_req (rem_use_env rx2a x) (lift_use_env rx2 (rx2a x)) (rx2a x)" and r_x="end_neg_use_env r_s2 x rx2 r" and
        r_c="end_neg_use_env r_s3 x rx2 r" in cancel_diff_use_env2)
        apply (rule_tac dist_end_neg_leq_use_env)
        apply (rule_tac well_typed_perm_leq)
        apply (auto)
    ...
    (* - with that, it's safe to activate the coercion lemma *)
      apply (rule_tac env="env" and ?rx1.0="rem_use_env r_s2 x" and ?e1.0="e2" and x="x" and r="r" in stpxpc_coercex)
              apply (auto)
         apply (rule_tac ?r_s1.0="add_use_env (rem_use_env r_s2 x) x (r_s2 x)" in well_typed_incr_start_perm)
          apply (rule_tac t="add_use_env (rem_use_env r_s2 x) x (r_s2 x)" and s="r_s2" in subst)
      ...
      apply (rule_tac well_typed_perm_leq)
      apply (auto)
     apply (rule_tac t="lift_use_env (diff_use_env (stpx_req (rem_use_env rx1a x) (lift_use_env rx2 (rx1a x)) (rx1a x)) (end_neg_use_env r_s2 x rx2 r)) ra"
      and s="diff_use_env (lift_use_env (stpx_req (rem_use_env rx1a x) (lift_use_env rx2 (rx1a x)) (rx1a x)) ra) (end_neg_use_env r_s2 x rx2 r)" in subst)
    ...
    (* - the issue here is that if r_s2 \<noteq> None, r_s3 = None, the lhs wont subtract but the rhs will. this only matters if rx1a has a value, in which
     ...
     apply (case_tac "rx1a x \<noteq> NoPerm")
      apply (case_tac "r_s3 x = NoPerm")
       apply (cut_tac r_x="rx1a" and r_s="r_s3" and x="x" in leq_use_none)
         apply (rule_tac r_sb="lift_use_env rx1a ra" in trans_leq_use_env)
          apply (simp)
         apply (rule_tac self_lift_leq_use_env)
        apply (auto)
      apply (case_tac "r_s2 x = NoPerm")
       apply (cut_tac r_x="r_s3" and r_s="r_s2" and x="x" in leq_use_none)
         apply (rule_tac well_typed_perm_leq)
         apply (auto)
      apply (simp add: end_neg_use_env_def)
      apply (rule_tac dist_diff_leq_use_env)
    (* - since rx1a x \<noteq> None, and lift rx1a ra x \<le> r, we have ra \<le> r *)
      apply (rule_tac r_sb="lift_use_env (comp_use_env (rem_use_env rx1a x) (lift_use_env rx2 (rx1a x))) ra" in trans_leq_use_env)
     ...
     apply (rule_tac disj_diff_leq_use_env)
      apply (rule_tac r_s="lift_use_env rx2 r" in disj_leq_use_env1)
       apply (rule_tac r_s="rx1" in disj_leq_use_env2)
        ...
        apply (simp)
       apply (rule_tac diff_leq_use_env)
       apply (rule_tac r_sb="rem_use_env r_s3 x" in trans_leq_use_env)
        apply (rule_tac r="r" in rem_add_leq_use_env)
        apply (simp)
       apply (simp add: lift_rem_use_env)
       apply (rule_tac dist_rem_leq_use_env)
    ...
    apply (rule_tac t="lift_use_env (diff_use_env (stpx_req (rem_use_env rx2a x) (lift_use_env rx2 (rx2a x)) (rx2a x)) (end_neg_use_env r_s3 x rx2 r)) ra"
      and s="diff_use_env (lift_use_env (stpx_req (rem_use_env rx2a x) (lift_use_env rx2 (rx2a x)) (rx2a x)) ra) (end_neg_use_env r_s3 x rx2 r)" in subst)
     apply (rule_tac lift_diff_use_env)
    apply (rule_tac dist_diff_leq_use_env)
    ...
    apply (rule_tac dist_rem_leq_use_env)
     ...
     apply (rule_tac r_s="comp_use_env (rem_use_env rx1a x) (stpx_ex (lift_use_env rx2 (rx1a x)) (rx1a x))" in safe_lift_leq_use_env)
      apply (rule_tac stpx_req_leq_use_env3)
      apply (rule_tac id_leq_use_env)
     apply (rule_tac safe_lift_comp_use_env)
      apply (rule_tac safe_lift_rem_use_env)
      apply (simp)
     apply (rule_tac r_x="rx1a" and x="x" in safe_lift_stpx_ex_use_env)
      apply (simp)
     apply (rule_tac r_s="rx1a" in self_safe_leq_perm)
     apply (simp)
    apply (rule_tac safe_lift_diff_use_env)
    ...
    apply (rule_tac safe_lift_comp_use_env)
     apply (rule_tac safe_lift_rem_use_env)
     apply (simp)
    apply (rule_tac r_x="rx2a" and x="x" in safe_lift_stpx_ex_use_env)
     apply (simp)
    apply (rule_tac r_s="rx2a" in self_safe_leq_perm)
     apply (simp)*)
   apply (rule_tac r_s="comp_use_env (lift_use_env (rem_use_env rx1a x) ra) (lift_use_env (stpx_ex (lift_use_env rx2 (rx1a x)) (rx1a x)) ra)" in disj_leq_use_env1)
    ...
    (* disjointness case analysis *)
     apply (case_tac "is_own ra")
      apply (case_tac "rx1a x \<noteq> NoPerm")
       apply (case_tac "rx2a x \<noteq> NoPerm")
        apply (case_tac "\<not> (\<forall>x. lift_use_env rx1a ra x = OwnPerm \<longrightarrow> lift_use_env rx2a ra x = NoPerm)")
         apply (simp add: disj_use_env_def)
         apply (simp add: mini_disj_use_env_def)
         apply (auto)
        apply (erule_tac x="x" in allE)
    ...
    (* - ra = Use *)
     ...
     apply (rule_tac self_lift_leq_use_env)
    (* - ra = Own *)
    apply (rule_tac t="comp_use_env (lift_use_env (rem_use_env rx2a x) ra) (lift_use_env (stpx_ex (lift_use_env rx2 (rx2a x)) (rx2a x)) ra)"
      and s="lift_use_env (comp_use_env (rem_use_env rx2a x) (stpx_ex (lift_use_env rx2 (rx2a x)) (rx2a x))) ra" in subst)
     apply (rule_tac lift_comp_use_env)
    apply (rule_tac dist_lift_leq_use_env)
    apply (rule_tac diff_leq_use_env)
    apply (rule_tac stpx_req_leq_use_env3)
   ...
   apply (rule_tac id_leq_use_env)
    (* final existentials *)
  apply (rule_tac x="comp_use_env (rem_use_env r_ex x) (end_neg_use_env r_se x rx2 r)" in exI)
  apply (auto)
     ...
     apply (rule_tac dist_comp_leq_use_env)
      ...
      apply (rule_tac disj_diff_leq_use_env)
       ...
       apply (rule_tac r_sb="diff_use_env r_s3 r_ex" in trans_leq_use_env)
        apply (rule_tac diff_leq_use_env)
        apply (auto)
      apply (rule_tac t="diff_use_env (rem_use_env r_s3 x) (rem_use_env r_ex x)" and
        s="rem_use_env (diff_use_env r_s3 r_ex) x" in subst)
       apply (rule_tac dist_diff_rem_use_env)
      apply (rule_tac dist_rem_leq_use_env)
      apply (simp)
     apply (rule_tac comp_leq_use_env2)
  ...
  apply (rule_tac r_sb="diff_use_env
           (comp_use_env (lift_use_env (diff_use_env (comp_use_env (rem_use_env rx1a x) (stpx_ex (lift_use_env rx2 (rx1a x)) (rx1a x))) (end_neg_use_env r_s2 x rx2 r)) ra)
             (lift_use_env (diff_use_env (comp_use_env (rem_use_env rx2a x) (stpx_ex (lift_use_env rx2 (rx2a x)) (rx2a x))) (end_neg_use_env r_s3 x rx2 r)) ra))
           (comp_use_env (rem_use_env r_ex x) (end_neg_use_env r_se x rx2 r))" in trans_leq_use_env)
   apply (rule_tac lhs_dist_dcl_use_env)
   apply (rule_tac dist_comp_leq_use_env)
    apply (rule_tac t="lift_use_env (diff_use_env (comp_use_env (rem_use_env rx1a x) (stpx_ex (lift_use_env rx2 (rx1a x)) (rx1a x))) (end_neg_use_env r_s2 x rx2 r)) ra"
      and s="diff_use_env (lift_use_env (comp_use_env (rem_use_env rx1a x) (stpx_ex (lift_use_env rx2 (rx1a x)) (rx1a x))) ra) (end_neg_use_env r_s2 x rx2 r)" in subst)
     apply (cut_tac r_s="comp_use_env (rem_use_env rx1a x) (stpx_ex (lift_use_env rx2 (rx1a x)) (rx1a x))" and r="ra"
      and r_x="(end_neg_use_env r_s2 x rx2 r)" in lift_diff_use_env)
     apply (auto)
    apply (simp add: lift_comp_use_env)
    ...
    apply (rule_tac dist_comp_leq_use_env)
     ...
     apply (rule_tac dist_diff_leq_use_env)
     apply (rule_tac comp_leq_use_env1)
     apply (rule_tac r_sb="rem_use_env (diff_use_env (comp_use_env (lift_use_env rx1a ra) (lift_use_env rx2a ra)) r_ex) x" in trans_leq_use_env)
      apply (rule_tac dist_rem_leq_use_env)
      apply (simp)
     apply (simp add: dist_diff_rem_use_env)
     apply (rule_tac dist_diff_leq_use_env)
    ...
    apply (rule_tac rhs_dist_dcl_use_env)
    apply (rule_tac comp_leq_use_env2)
    apply (rule_tac r_sb="diff_use_env (lift_use_env (stpx_ex (lift_use_env rx2 (rx1a x)) (rx1a x)) ra) (end_neg_use_env r_se x rx2 r)" in trans_leq_use_env)
    (* - rx1a x \<noteq> None *)
     apply (case_tac "rx1a x = NoPerm")
      apply (rule_tac diff_leq_use_env)
      apply (simp add: stpx_ex_def)
    ...
    (* - if r_ex = Own, r = Own and r_se x = None *)
     apply (case_tac "r_ex x = OwnPerm")
      apply (case_tac "\<not> is_own r")
       apply (cut_tac r_x="r_ex" and r_s="add_use_env rx1 x r" and x="x" in leq_use_own)
         apply (simp_all)
       apply (simp add: add_use_env_def)
       apply (simp add: is_own_def)
      apply (cut_tac r_x="r_se" and r_s="diff_use_env r_s3 r_ex" and x="x" in leq_use_none)
        apply (simp)
       apply (rule_tac diff_use_none_ex)
       apply (auto)
    ...
    (* - if r_xe x = Own, trivial *)
     apply (case_tac "is_own (r_xe x)")
      apply (case_tac "stpx_ex (lift_use_env rx2 (r_xe x)) (r_xe x) \<noteq> lift_use_env rx2 (r_xe x)")
       apply (simp add: is_own_def)
       apply (simp add: stpx_ex_def)
      apply (auto)
      apply (rule_tac dist_diff_leq_use_env)
    ...
    (* - if ra = Own, then r_ex = Own, since (lift rx1a ra) - r_ex \<le> r_xe *)
     apply (case_tac "is_own ra")
      apply (cut_tac r_x="diff_use_env (lift_use_env rx1a ra) r_ex" and r_s="r_xe" and x="x" in leq_use_own)
        apply (cut_tac r_x="lift_use_env rx1a ra" and r_s="r_xe" and r_ex="r_ex" and x="x" in diff_use_leq)
          apply (simp)
    ...
    (* if ra \<noteq> Own, we can ignore it. now we should have rx1a \<le> r_xe *)
     apply (case_tac "ra \<noteq> UsePerm")
      ...
      apply (case_tac ra)
        apply (auto)
     apply (cut_tac r_x="rx1a" and r_s="r_xe" and r_ex="r_ex" and x="x" in diff_use_leq)
       apply (auto)
   ...
   apply (rule_tac t="lift_use_env (diff_use_env (comp_use_env (rem_use_env rx2a x) (stpx_ex (lift_use_env rx2 (rx2a x)) (rx2a x))) (end_neg_use_env r_s3 x rx2 r)) ra"
      and s="diff_use_env (lift_use_env (comp_use_env (rem_use_env rx2a x) (stpx_ex (lift_use_env rx2 (rx2a x)) (rx2a x))) ra) (end_neg_use_env r_s3 x rx2 r)" in subst)
    apply (cut_tac r_s="comp_use_env (rem_use_env rx2a x) (stpx_ex (lift_use_env rx2 (rx2a x)) (rx2a x))" and r="ra"
      and r_x="(end_neg_use_env r_s3 x rx2 r)" in lift_diff_use_env)
    apply (auto)
   apply (simp add: lift_comp_use_env)
   apply (rule_tac lhs_fold_dcl_use_env)
   apply (rule_tac lhs_dist_dcl_use_env)
   ...
   apply (rule_tac rhs_dist_dcl_use_env)
   apply (rule_tac comp_leq_use_env2)
   apply (rule_tac r_sb="diff_use_env (lift_use_env (stpx_ex (lift_use_env rx2 (rx2a x)) (rx2a x)) ra) (end_neg_use_env r_se x rx2 r)" in trans_leq_use_env)
    (* - rx2a x \<noteq> None *)
    ...
    apply (case_tac "is_own ra")
     apply (cut_tac r_x="diff_use_env (lift_use_env rx2a ra) r_ex" and r_s="r_xe" and x="x" in leq_use_own)
       apply (cut_tac r_x="lift_use_env rx2a ra" and r_s="r_xe" and r_ex="r_ex" and x="x" in diff_use_leq)
         apply (simp)
        apply (rule_tac r_sb="diff_use_env (comp_use_env (lift_use_env rx1a ra) (lift_use_env rx2a ra)) r_ex" in trans_leq_use_env)
         apply (simp)
        apply (rule_tac dist_diff_leq_use_env)
    ...
    apply (case_tac "ra \<noteq> UsePerm")
     ...
     apply (case_tac ra)
       apply (auto)
    apply (cut_tac r_x="rx2a" and r_s="r_xe" and r_ex="r_ex" and x="x" in diff_use_leq)
      apply (auto)
     apply (rule_tac r_sb="diff_use_env (comp_use_env rx1a rx2a) r_ex" in trans_leq_use_env)
      apply (simp)
     apply (rule_tac dist_diff_leq_use_env)
...
lemma stpx_if_no_fv: "\<lbrakk>
     leq_use_env (rem_use_env r_s1 x) rx1; disj_use_env rx1 (lift_use_env rx2 r); lam_vars e \<inter> (free_vars e' \<union> ref_vars e') = {};
     well_typed (add_env env x t) r_s1 e tau r_s2 rx'; leq_perm (rx' x) r; x \<notin> free_vars e; x \<notin> ref_vars e \<rbrakk>
  ...
  apply (auto)
   ...
   apply (case_tac "(rem_use_env r_s1 x) xa = NoPerm")
    apply (cut_tac env="add_env env x t" and ?r_s1.0="r_s1" and e="e" and x="xa" in well_typed_no_npv_use)
      apply (auto)
     apply (simp add: rem_use_env_def)
    apply (simp add: non_prim_vars_def)
    apply (simp add: non_prim_entry_def)
...
lemma stpx_if_case: "\<lbrakk>\<And>env x tau t rx1 r_se r_xe r_s1 rx2 r.
        ...
        \<And>env x tau t rx1 r_se r_xe r_s1 rx2 r.
           \<lbrakk>well_typed (add_env env x t) (add_use_env rx1 x r) e3 tau r_se r_xe; well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; x \<in> free_vars e3;
            x \<notin> ref_vars e3; lam_vars e3 \<inter> (free_vars e' \<union> ref_vars e') = {}; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1;
            disj_use_env rx1 (lift_use_env rx2 r)\<rbrakk>
           \<Longrightarrow> well_typed env r_s1 (subst_exp e3 x e') tau
                (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r))
                (diff_use_env (stpx_req (rem_use_env r_xe x) (lift_use_env rx2 (r_xe x)) (r_xe x)) (end_neg_use_env r_se x rx2 r));
        well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; is_sexp e';
        (lam_vars e1 \<union> lam_vars e2 \<union> lam_vars e3) \<inter> (free_vars e' \<union> ref_vars e') = {}; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1;
        disj_use_env rx1 (lift_use_env rx2 r); x \<notin> ref_vars e1; well_typed (add_env env x t) (add_use_env rx1 x r) e1 BoolTy r_s2 rx'; x \<notin> ref_vars e2;
        x \<notin> ref_vars e3; well_typed (add_env env x t) r_s2 e2 tau r_se rx1a; well_typed (add_env env x t) r_s2 e3 tau r_se rx2a\<rbrakk>
       \<Longrightarrow> \<exists>rx' r_s2.
              well_typed env r_s1 (subst_exp e1 x e') BoolTy r_s2 rx' \<and>
  ...
  apply (auto)
   ...
   apply (rule_tac t="diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r)" and
      s="diff_use_env (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r))
      (end_neg_use_env r_s2 x rx2 r)" in subst)
    apply (cut_tac r_s="comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)" and r_c="end_neg_use_env r_se x rx2 r" and
      r_x="end_neg_use_env r_s2 x rx2 r" in cancel_diff_use_env2)
     apply (auto)
   apply (rule_tac t="diff_use_env (stpx_req (rem_use_env rx1a x) (lift_use_env rx2 (rx1a x)) (rx1a x)) (end_neg_use_env r_se x rx2 r)" and
      s="diff_use_env (diff_use_env (stpx_req (rem_use_env rx1a x) (lift_use_env rx2 (rx1a x)) (rx1a x)) (end_neg_use_env r_se x rx2 r))
      (end_neg_use_env r_s2 x rx2 r)" in subst)
    apply (cut_tac r_s="stpx_req (rem_use_env rx1a x) (lift_use_env rx2 (rx1a x)) (rx1a x)" and r_c="end_neg_use_env r_se x rx2 r" and
      r_x="end_neg_use_env r_s2 x rx2 r" in cancel_diff_use_env2)
     apply (auto)
   apply (rule_tac well_typed_diff_perms)
    ...
    apply (case_tac "lam_vars e2 \<inter> (free_vars e' \<union> ref_vars e') = {}")
     apply (auto)
  ...
  apply (auto)
   ...
   apply (rule_tac t="diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r)" and
    ...
    apply (cut_tac r_s="comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)" and r_c="end_neg_use_env r_se x rx2 r" and
      r_x="end_neg_use_env r_s2 x rx2 r" in cancel_diff_use_env2)
     apply (auto)
   apply (rule_tac t="diff_use_env (stpx_req (rem_use_env rx2a x) (lift_use_env rx2 (rx2a x)) (rx2a x)) (end_neg_use_env r_se x rx2 r)" and
      s="diff_use_env (diff_use_env (stpx_req (rem_use_env rx2a x) (lift_use_env rx2 (rx2a x)) (rx2a x)) (end_neg_use_env r_se x rx2 r))
      (end_neg_use_env r_s2 x rx2 r)" in subst)
    apply (cut_tac r_s="stpx_req (rem_use_env rx2a x) (lift_use_env rx2 (rx2a x)) (rx2a x)" and r_c="end_neg_use_env r_se x rx2 r" and
      r_x="end_neg_use_env r_s2 x rx2 r" in cancel_diff_use_env2)
     apply (auto)
   apply (rule_tac well_typed_diff_perms)
    (* induction requirements *)
    apply (case_tac "\<not> rem_use_env r_s2 x x = NoPerm")
     apply (simp add: rem_use_env_def)
    apply (cut_tac r="comp_use_env (rem_use_env r_s2 x) (lift_use_env rx2 r)" in id_leq_use_env)
    apply (case_tac "lam_vars e3 \<inter> (free_vars e' \<union> ref_vars e') = {}")
     apply (auto)
    apply (cut_tac r_x="rem_use_env r_s2 x" and r_s="rx1" and r_ex="lift_use_env rx2 r" in disj_leq_use_env1)
      apply (simp)
     apply (rule_tac r="r" in rem_add_leq_use_env)
...
(*
lemma stpalc_drl_use_env2: "\<lbrakk> leq_use_env (diff_use_env r_x r_ex) r_s \<rbrakk> \<Longrightarrow>
  leq_use_env (diff_use_env r_x (rem_use_env r_ex x)) (rem_use_env r_s x)"
  apply (simp add: leq_use_env_def)
  apply (simp add: rem_use_env_def)
  apply (simp add: diff_use_env_def)
  apply (simp add: minus_use_env_def)
...
lemma stpalc_nal_use_env: "\<lbrakk> r_s x = NoPerm; leq_use_env r_x (add_use_env r_s x r) \<rbrakk> \<Longrightarrow> leq_use_env (norm_use_env r_x r_s) r_s"    
  ...
  apply (case_tac "x = xa")
   apply (auto)
    ...
    (*
lemma wt_sexp_no_perm: "\<lbrakk> well_typed env rx e tau rx rx; is_sexp e; req_type tau = Prim \<rbrakk> \<Longrightarrow>
  well_typed env empty_use_env e tau empty_use_env empty_use_env"    
  apply (case_tac e)
        apply (auto)
    (* const + op case *)
        apply (rule_tac leq_empty_use_env)
  ...
  apply (rule_tac leq_empty_use_env)
    ...
    (*
lemma wt_sexp_add_vars: "\<lbrakk> well_typed env r_s1 e tau r_s2 rx; is_sexp e; req_type tau = Prim;
  lam_vars e \<inter> free_vars e' = {} \<rbrakk> \<Longrightarrow> well_typed (add_env env x t) r_s1 e tau r_s2 rx"    
  apply (case_tac e)
       apply (auto)
    (* var case *)
    apply (simp add: add_env_def)
    ...
    apply (case_tac tau)
          apply (auto)
    apply (case_tac "req_type x51 = Aff \<or> req_type x52 = Aff")
     apply (auto)
    (* lam case *)
   apply (rule_tac x="rxa" in exI)
   apply (auto)
   apply (rule_tac x="r_end" in exI)
   apply (rule_tac x="r_s'" in exI)
   apply (case_tac "x = x51")
    apply (cut_tac env="env" and x="x" and t="t" and t'="t1" in double_add_env)
    apply (auto)
   apply (cut_tac env="env" and x="x51" and y="x" and t="t" and t'="t1" in almost_comm_add_env)
    apply (auto)
...
lemma stpx_lam_case: "\<lbrakk>\<And>env x tau t rx1 r_se r_xe r_s1 rx2 r.
           \<lbrakk>well_typed (add_env env x t) (add_use_env rx1 x r) e tau r_se r_xe; well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; x \<in> free_vars e;
            x \<notin> ref_vars e; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1; disj_use_env rx1 (lift_use_env rx2 r)\<rbrakk>
           \<Longrightarrow> well_typed env r_s1 (subst_exp e x e') tau
                (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (end_neg_use_env r_se x rx2 r))
                (diff_use_env (stpx_req (rem_use_env r_xe x) (lift_use_env rx2 (r_xe x)) (r_xe x)) (end_neg_use_env r_se x rx2 r));
        well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; x \<notin> ref_vars e; is_sexp e'; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1;
        disj_use_env rx1 (lift_use_env rx2 r); x \<in> free_vars e; x \<noteq> x1a; x1a \<notin> free_vars e'; x1a \<notin> ref_vars e';
        lam_vars e \<inter> (free_vars e' \<union> ref_vars e') = {}; x1a \<notin> ref_vars e;
  ...
  apply (case_tac "rx x = NoPerm")
   apply (cut_tac env="add_env (add_env env x t) x1a t1" and ?r_s1.0="add_use_env rx x1a ra" and x="x" and e="e" in well_typed_no_npv_use)
     apply (auto)
    apply (simp add: add_use_env_def)
   ...
   apply (auto)
     ...
     apply (rule_tac t="t" and ?rx2.0="rx2" in subst_type_preserve_prim_fv)
           apply (auto)
      apply (cut_tac env="env" and x="x" and y="x1a" and t="t1" and t'="t" in almost_comm_add_env)
       apply (auto)
     apply (rule_tac well_typed_add_vars)
       apply (auto)
    apply (rule_tac r_sb="comp_use_env rx1 (lift_use_env rx2 r)" in trans_leq_use_env)
     apply (simp)
    apply (rule_tac comp_leq_use_env1)
    apply (rule_tac r_sb="rem_use_env rx x" in trans_leq_use_env)
     apply (rule_tac r="r" in rem_add_leq_use_env)
     apply (simp)
   ...
   apply (cut_tac r_s="rx" and x="x" in ignore_rem_use_env)
    apply (auto)
   apply (rule_tac t="diff_use_env rx (rem_use_env r_ex x)" and s="rem_use_env (diff_use_env rx r_ex) x" in subst)
    apply (cut_tac r_s="rx" and x="x" and r_x="r_ex" in dist_diff_rem_use_env)
    apply (auto)
   apply (rule_tac dist_rem_leq_use_env)
    apply (simp)
    ...
    *)
    ...
    (* - case analysis to prove rx x = None is impossible, rx x = Use implies unlim t *)
       ...
       apply (case_tac "rx x")
         apply (auto)*)
       apply (auto)
       apply (cut_tac env="add_env (add_env env x t) x1a t1" and ?r_s1.0="add_use_env rx x1a ra" and e="e" and x="x" and t="t" in well_typed_aff_fv_own)
           apply (auto)
        apply (simp add: add_env_def)
       apply (simp add: add_use_env_def)
...
lemma subst_type_preserve_x: "\<lbrakk> well_typed (add_env env x t) (add_use_env rx1 x r) e tau r_se r_xe;
  well_typed env rx2 e' t rx2 rx2; rx1 x = NoPerm; x \<in> free_vars e; x \<notin> ref_vars e; is_sexp e'; (*r \<noteq> NoPerm;*)
  lam_vars e \<inter> (free_vars e' \<union> ref_vars e') = {}; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s1;
