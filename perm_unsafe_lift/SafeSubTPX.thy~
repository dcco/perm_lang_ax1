theory SafeSubTPX
  imports SuperNormEnv SubstTPX SafeSubRename SubstFlatLemma
begin  
...
lemma squish_leq_use_env: "\<lbrakk> leq_use_env r_se (diff_use_env r_s r_ex); leq_use_env r_xe r_se; leq_use_env (diff_use_env rxa r_ex) r_xe \<rbrakk> \<Longrightarrow>
  ...
  (*
lemma safe_lift_refl_use_env: "\<lbrakk> safe_use_lift r_s r \<rbrakk> \<Longrightarrow> safe_use_lift (refl_use_env r_s1 r_s2 r) r"    
  apply (case_tac r)
    apply (auto)
...
lemma lift_refl_use_env: "lift_use_env (refl_use_env r_s1 r_s2 r) r = refl_use_env r_s1 r_s2 r"    
  apply (case_tac "\<forall> x. lift_use_env (refl_use_env r_s1 r_s2 r) r x = refl_use_env r_s1 r_s2 r x")
   apply (auto)
  apply (case_tac r)
    apply (auto)
  apply (simp add: refl_use_env_def)
...
lemma subst_type_preserve_app: "\<lbrakk> well_typed (add_env env x t) (add_use_env rx1 x r) e tau r_se r_xe;
  well_typed env r_s2 e' t r_s3 rx2; rx1 x = NoPerm; x \<in> free_vars e; x \<notin> ref_vars e; is_sexp e';
  lam_vars e \<inter> (free_vars e' \<union> ref_vars e') = {}; leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s3;
  r \<noteq> NoPerm; (*safe_use_lift rx2 r; safe_type t r;*) disj_use_env rx1 (lift_use_env rx2 r); leq_use_env r_s2 r_s1; leq_use_env rx1 r_s1 \<rbrakk> \<Longrightarrow>
  ...
  apply (cut_tac r_sc="r_xe" and r_sb="r_se" and r_sa="add_use_env rx1 x r" in trans_leq_use_env)
  ...
  apply (rule_tac ?r_s2.0="comp_use_env (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r))
    ...
    (diff_use_env r_s3 (comp_use_env rx1 (lift_use_env (comp_use_env rx2 (infl_use_env r_s2 r_s3)) r)))" and
      rx="diff_use_env (comp_use_env (rem_use_env r_xe x) (lift_use_env rx2 (r_xe x)))
    ...
    (* req: ((r_xe - x) + rx2) - rx' >> (((r_xe - x) + rx') - (rx1 + rx')) - r_ex *)
       apply (rule_tac ?r_s2.0="diff_use_env (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env (comp_use_env rx2 (infl_use_env r_s2 r_s3)) r))
          (comp_use_env rx1 (lift_use_env (comp_use_env rx2 (infl_use_env r_s2 r_s3)) r))) (infl_use_env r_s2 r_s3)" and
          rx="diff_use_env (diff_use_env (stpx_req (rem_use_env r_xe x) (lift_use_env (comp_use_env rx2 (infl_use_env r_s2 r_s3)) (r_xe x)) (r_xe x))
    ...
    (* end_neg lift *)
           apply (rule_tac r_exb="end_neg_use_env r_se x (comp_use_env rx2 (infl_use_env r_s2 r_s3)) r" in well_typed_diff_decr_end_perm_gen)
...
(*
    ...
    (* requirements transformation: first we want to remove stpx_req *)
       apply (cut_tac r_s="rx2" and r_x="infl_use_env r_s2 r_s3" and r="r_xe x" in lift_comp_use_env)
       apply (simp)
       apply (rule_tac r_sb="diff_use_env (diff_use_env (comp_use_env (rem_use_env r_xe x) (comp_use_env (lift_use_env rx2 (r_xe x)) (lift_use_env (infl_use_env r_s2 r_s3) (r_xe x))))
...
(*
  ...
  apply (rule_tac self_comp_leq_use_env1)
    ...
    (*
lemma subst_type_preserve_app: "\<lbrakk> well_typed (add_env env x t) (add_use_env rx1 x r) e tau r_se r_xe;
  well_typed env r_s2 e' t r_s3 rx2; rx1 x = NoPerm; x \<in> free_vars e; is_sexp e';
  ...
  well_typed env r_s1 (subst_exp e x e') tau (diff_use_env r_s3 (comp_use_env rx1 (lift_use_env rx2 r)))
    (diff_use_env (comp_use_env rx1 rx2) (comp_use_env rx1 (lift_use_env rx2 r)))"
  apply (cut_tac env="env" and ?r_s1.0="r_s2" and e="e'" and tau="t" and ?r_s2.0="r_s3" and rx="rx2" and rx_p="rx1" and r="r" in sexp_wild_perm)
       apply (auto)
     apply (rule_tac r_sb="comp_use_env rx1 (lift_use_env rx2 r)" in trans_leq_use_env)
      apply (simp)
     apply (rule_tac self_comp_leq_use_env2)
    ...
    apply (rule_tac self_comp_leq_use_env1)
   apply (rule_tac comm_disj_use_env)
   apply (rule_tac r="r" in disj_lift_use_envr)
   apply (simp)
    (* prelim: r_xe \<le> rx1 + x *)
  apply (cut_tac r_sc="r_xe" and r_sb="r_se" and r_sa="add_use_env rx1 x r" in trans_leq_use_env)
    apply (rule_tac well_typed_perm_leq)
  ...
  apply (simp)
    (* end_perm: r_s3 - (rx1 + rx2) << (((r_se - x) + rx2) - (rx1 + rx')) + (r_s3 - (rx1 + rx')) *)
    (* req: (rx1 + rx2) - (rx1 + rx2) >> ((r_xe - x) + rx2) - (rx1 + rx')*)
  apply (rule_tac ?r_s2.0="comp_use_env (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx2 r)) (comp_use_env rx1 (lift_use_env rx' r)))
    (diff_use_env r_s3 (comp_use_env rx1 (lift_use_env rx' r)))" and
      rx="diff_use_env (comp_use_env (rem_use_env r_xe x) (lift_use_env rx2 (r_xe x))) (comp_use_env rx1 (lift_use_env rx' r))" in well_typed_simul_end_perm)
    (* start_perm: r_s1 >> (rx1 + rx' + r_ex) + (r_s3 - (rx1 + rx')) *)
     apply (rule_tac ?r_s1.0="comp_use_env (comp_use_env (comp_use_env rx1 (lift_use_env rx' r)) r_ex)
      (diff_use_env r_s3 (comp_use_env rx1 (lift_use_env rx' r)))" in well_typed_incr_start_perm)
    (* comp_cancel into start_perm: r_s1, end_perm: ((r_se - x) + rx2) - (rx1 + rx') *)
      apply (rule_tac well_typed_comp_perms_gen)
    (* end_perm: ((r_se - x) + rx2) - rx' << (((r_se - x) + rx') - (rx1 + rx')) - r_ex. *)
    (* req: ((r_xe - x) + rx2) - rx' >> (((r_xe - x) + rx') - (rx1 + rx')) - r_ex *)
       apply (rule_tac ?r_s2.0="diff_use_env (diff_use_env (comp_use_env (rem_use_env r_se x) (lift_use_env rx' r)) (comp_use_env rx1 (lift_use_env rx' r))) r_ex" and
          rx="diff_use_env (diff_use_env (stpx_req (rem_use_env r_xe x) (lift_use_env rx' (r_xe x)) (r_xe x)) (comp_use_env rx1 (lift_use_env rx' r))) r_ex" in well_typed_simul_end_perm)
    (* diff_cancel into end_perm: ((r_se - x) + rx') - (rx1 + rx'), req: ((r_xe - x) + rx') - (rx1 + rx') *)
          apply (rule_tac well_typed_diff_end_perm)
    (* end_neg lift *)
           apply (rule_tac r_exb="end_neg_use_env r_se x rx' r" in well_typed_diff_decr_end_perm_gen)
             apply (rule_tac t="t" and ?rx1.0="rx1" in subst_type_preserve_x)
                      apply (auto)
    (* basic stpx requirements *)
             apply (rule_tac self_comp_leq_use_env1)
...
lemma diff_norm_leq_use_env: "\<lbrakk> leq_use_env r_ex r_s; leq_use_env r_x (diff_use_env r_s r_ex) \<rbrakk> \<Longrightarrow> leq_use_env (norm_use_env r_s r_x) (diff_use_env r_s r_ex)"    
  apply (simp add: leq_use_env_def)
  apply (simp add: norm_use_env_def)
  apply (auto)
  apply (simp add: diff_use_env_def)
...
lemma dist_diff_norm_leq_use_env: "\<lbrakk> leq_use_env r_ex r_s; leq_use_env r_xb (diff_use_env r_xa r_ex) \<rbrakk> \<Longrightarrow>
  leq_use_env (norm_use_env r_s r_xb) (diff_use_env (norm_use_env r_s r_xa) r_ex)"    
  apply (simp add: leq_use_env_def)
  apply (simp add: norm_use_env_def)
  apply (auto)
  apply (simp add: diff_use_env_def)
...
lemma dist_norm_leq_use_env: "\<lbrakk> leq_use_env r_ex r_exa; leq_use_env r_ex r_exb \<rbrakk> \<Longrightarrow> leq_use_env (norm_use_env r_s r_ex) (norm_use_env (norm_use_env r_s r_exa) r_exb)"
  ...
  (*
lemma well_typed_str_end_perm: "\<lbrakk> well_typed env r_s1 e tau r_s2 rx \<rbrakk> \<Longrightarrow> well_typed env r_s1 e tau (norm_use_env r_s1 r_s2) rx"   
  apply (induct e arbitrary: env r_s1 tau r_s2 rx)
       apply (auto)
    (* const + op cases *)
          apply (rule_tac self_norm_leq_use_env)
         apply (rule_tac rhs_norm_leq_use_env2)
          apply (auto)
        apply (rule_tac self_norm_leq_use_env)
       apply (rule_tac rhs_norm_leq_use_env2)
        apply (auto)
    ...
    apply (rule_tac x="rx1" in exI)
    apply (auto)
     apply (rule_tac r_c="norm_use_env (norm_use_env r_s1 r_s2a) r_s2" in well_typed_decr_end_perm)
       apply (cut_tac env="env" and ?r_s1.0="r_s2a" and r_c="norm_use_env r_s1 r_s2a" and ?r_s2.0="r_s2" and e="e2" in well_typed_incr_start_perm)
         apply (auto)
       apply (rule_tac rhs_norm_leq_use_env)
        apply (rule_tac well_typed_perm_leq)
        apply (auto)
       apply (rule_tac id_leq_use_env)
      apply (rule_tac dist_norm_leq_use_env)
       apply (rule_tac well_typed_perm_leq)
       apply (auto)
      apply (rule_tac id_leq_use_env)
     apply (rule_tac rhs_norm_leq_use_env2)
    ...
    apply (auto)
    apply (rule_tac r_c="norm_use_env (norm_use_env r_s1 r_s2a) r_s2" in well_typed_decr_end_perm)
      apply (cut_tac env="env" and ?r_s1.0="r_s2a" and r_c="norm_use_env r_s1 r_s2a" and ?r_s2.0="r_s2" and e="e3" in well_typed_incr_start_perm)
        apply (auto)
      apply (rule_tac rhs_norm_leq_use_env)
       apply (rule_tac well_typed_perm_leq)
       apply (auto)
  ...
  apply (auto)
  apply (rule_tac x="rx2" in exI)
  apply (rule_tac x="norm_use_env (norm_use_env r_s1 r_s2a) r_s3" in exI)
  apply (auto)
   apply (cut_tac env="env" and ?r_s1.0="r_s2a" and r_c="norm_use_env r_s1 r_s2a" and ?r_s2.0="r_s3" and e="e2" in well_typed_incr_start_perm)
     apply (auto)
   apply (rule_tac rhs_norm_leq_use_env)
    apply (rule_tac well_typed_perm_leq)
    apply (auto)
  ...
  apply (rule_tac x="r_ex" in exI)
  apply (auto)
    apply (rule_tac r_sb="diff_use_env (norm_use_env r_s1 r_s3) (comp_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_ex)" in trans_leq_use_env)
     apply (rule_tac dist_diff_leq_use_env)
     apply (rule_tac dist_norm_leq_use_env)
      apply (rule_tac well_typed_perm_leq)
      apply (auto)
     apply (rule_tac id_leq_use_env)
    apply (rule_tac dist_diff_norm_leq_use_env)
     apply (rule_tac dist_comp_leq_use_env)
      apply (rule_tac r_sb="r_s3" in trans_leq_use_env)
       apply (rule_tac r_sb="r_s2a" in trans_leq_use_env)
        apply (rule_tac well_typed_perm_leq)
...
lemma wtepl_diff_remove_lemma: "\<lbrakk> diff_use_env r_s1 r_ex = super_norm_use_env r_s1 r_s2a; leq_use_env r_s2a r_s1; r_ex x = OwnPerm \<rbrakk> \<Longrightarrow> r_s2a x = NoPerm"    
  apply (case_tac "super_norm_use_env r_s1 r_s2a x = minus_use_env r_s1 (neg_use_env r_ex) x")
    ...
    (* var case *)
      ...
      apply (auto)
        ...
        apply (rule_tac r_sb="r_s2" in trans_leq_use_env)
         apply (auto)
       apply (rule_tac id_leq_use_env)
      apply (rule_tac r_sb="diff_use_env (ereq_use_env x1a tau_x) (comp_use_env (comp_use_env (ereq_use_env x1a tau_x) r_ex) r_s1)" in trans_leq_use_env)
...
lemma sstp_add_perms: "\<lbrakk> well_typed env (add_use_env r_s1 x r) e tau r_s2 rx;
  ...
  apply (rule_tac self_rem_leq_use_env)
    ...
    *)
lemma safe_subst_type_preserve_x: "\<lbrakk> well_typed (add_env env x t) (add_use_env rx1 x r) e1 tau r_s' r_end; safe_subst_exp e1 x e' e2;
  is_sexp e'; well_typed env r_s2 e' t r_s3 rx2; leq_use_env r_s2 r_s1; leq_use_env rx1 r_s1;
  leq_use_env (comp_use_env rx1 (lift_use_env rx2 r)) r_s3; r \<noteq> NoPerm; (*safe_use_lift rx2 r; safe_type t r;*) disj_use_env rx1 (lift_use_env rx2 r) \<rbrakk> \<Longrightarrow>
  ...
  apply (case_tac "x \<in> free_vars e1")
   apply (rule_tac ?r_s2.0="diff_use_env r_s3 (comp_use_env (rem_use_env rx1 x) (lift_use_env rx2 r))" and
        rx="diff_use_env (comp_use_env (rem_use_env rx1 x) rx2) (comp_use_env (rem_use_env rx1 x) (lift_use_env rx2 r))" in well_typed_simul_end_perm)
    thm subst_type_preserve_app
        apply (rule_tac subst_type_preserve_app)
                    apply (auto)
                apply (cut_tac r_s="rx1" and x="x" and r="r" in partial_add_rem_use_env)
                apply (auto)
    ...
    *)
    ...
    (* prove removing from rx1 is acceptable *)
     ...
     apply (rule_tac dist_comp_leq_use_env)
      apply (rule_tac comp_leq_use_env1)
      apply (rule_tac t="diff_use_env (rem_use_env rx1 x) (rem_use_env rx1 x)" and
        s="rem_use_env (diff_use_env rx1 rx1) x" in subst)
    ...
    apply (rule_tac ?r_s1.0="comp_use_env rx1 (diff_use_env r_s3 rx1)" in well_typed_incr_start_perm)
     apply (rule_tac ?r_s2.0="comp_use_env (diff_use_env rx1 rx1) (diff_use_env r_s3 rx1)" and
       rx="diff_use_env (rem_use_env r_end x) rx1" in well_typed_simul_end_perm)
        apply (rule_tac well_typed_comp_perms_gen)
         apply (rule_tac ?r_s2.0="rem_use_env r_s' x" in well_typed_end_perm_lbound)
    ...
    (*
lemma safe_subst_type_preserve: "\<lbrakk> well_typed (add_env env x t) (add_use_env rx1 x r) e1 tau r_s' r_end; safe_subst_exp e1 x e' e2;
  is_sexp e'; well_typed env r_s2 e' t r_s3 rx2; leq_use_env r_s2 r_s1; leq_use_env rx1 r_s1;
  ...
  (\<exists> r_xe r_se. well_typed env r_s1 e2 tau r_se r_xe)"
  apply (simp add: safe_subst_exp_def)
  apply (auto)
  apply (case_tac "x \<in> free_vars e1")
   apply (rule_tac x="diff_use_env (comp_use_env (rem_use_env rx1 x) rx2) (comp_use_env (rem_use_env rx1 x) (lift_use_env rx2 r))" in exI)
   apply (rule_tac x="diff_use_env r_s3 (comp_use_env (rem_use_env rx1 x) (lift_use_env rx2 r))" in exI)
   apply (rule_tac t="t" and ?r_s1.0="r_s1" and ?r_s2.0="r_s2" and r_se="r_s'" and r_xe="r_end" in subst_type_preserve_app)
               apply (auto)
           apply (cut_tac r_s="rx1" and x="x" and r="r" in partial_add_rem_use_env)
           apply (auto)
           apply (rule_tac lam_var_list_remove_type_preserve)
  ...
  apply (rule_tac ?r_s1.0="rem_use_env rx1 x" in well_typed_incr_start_perm)
   apply (rule_tac x="x" and t="t" in well_typed_add_vars_rev)
    apply (auto)
