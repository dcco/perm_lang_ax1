...
lemma safe_val_exp: "\<lbrakk> well_typed env r_s1 (app_hole h (AppExp (AppExp (ConstExp SendConst) (VarExp x a)) v)) tau r_s2 rx;
  env (deref_name x a) = Some (ChanTy t SEnd); is_value c; is_value v \<rbrakk> \<Longrightarrow>
  ...
  apply (rule_tac t="np_dom_use_env env v" and s="diff_use_env (comp_use_env (lift_use_env rx2 r) (infl_use_env r_s2a r_s3))
   ...
   apply (simp add: leq_use_env_def)
   apply (auto)
   apply (case_tac "comp_use_env (lift_use_env rx2 r) (infl_use_env r_s2a r_s3) xa \<noteq> OwnPerm")
    apply (case_tac "comp_use_env rx2 (infl_use_env r_s2a r_s3) xa = NoPerm")
     apply (cut_tac x="xa" and env="env" and e="v" and ?r_s1.0="comp_use_env rx2 (infl_use_env r_s2a r_s3)" in well_typed_no_npv_use)
       apply (auto)
   apply (cut_tac r_sa="lift_use_env rx2 r" and r_sb="infl_use_env r_s2a r_s3" and x="xa" in comp_use_no_own_both)
    apply (auto)
   apply (case_tac "rx2 xa \<noteq> NoPerm")
    apply (simp add: is_own_def)
   apply (auto)
   apply (case_tac "infl_use_env r_s2a r_s3 xa \<noteq> NoPerm")
    apply (simp add: infl_use_env_def)
    apply (case_tac "r_s2a xa = OwnPerm \<and> r_s3 xa = NoPerm")
     apply (auto)
   apply (cut_tac r_sa="rx2" and r_sb="infl_use_env r_s2a r_s3" and x="xa" in comp_use_none)
     apply (auto)
  ...
  apply (auto)
  apply (case_tac "np_dom_use_env env v xa \<noteq> OwnPerm")
   apply (simp add: np_dom_use_env_def)
   apply (simp add: dom_use_env_def)
   apply (cut_tac r_s="lift_use_env (comp_use_env (lift_use_env rx2 r) (infl_use_env r_s2a r_s3)) r" and
       r_ex="np_dom_use_env env v" and x="xa" in diff_use_none_ex)
    apply (simp)
  apply (simp add: own_env_vars_def)
  done
    
  
lemma safe_recv_exp: "\<lbrakk> well_typed env r_s1 (app_hole h (AppExp (ConstExp RecvConst) (VarExp x a))) tau r_s2 rx;
  env (deref_name x a) = Some (ChanTy t REnd); well_typed env r_x v t r_x r_x; disj_use_env r_s1 r_x; 
...
lemma safe_own_npv_use: "\<lbrakk> well_typed env r_s1 (app_hole h (AppExp f v)) tau r_s2 rx; is_value f; strong_fun f;
  ...
  (*
lemma safe_send_own_npv_use: "\<lbrakk> well_typed env r_s1 (app_hole h (AppExp (AppExp (ConstExp SendConst) (VarExp c a)) v)) tau r_s2 rx;
  a \<noteq> NoRef; is_value v; x \<in> non_prim_vars env v \<rbrakk> \<Longrightarrow> r_s1 x = OwnPerm"
  apply (induct h arbitrary: env r_s1 r_s2 tau rx)
       apply (auto)
    (* base case *)
    apply (cut_tac r_sc="r_s2a" and r_sb="diff_use_env r_s3a (comp_use_env (comp_use_env rx1a (lift_use_env rx2a ra)) r_exb)" and
      r_sa="r_s1" in trans_leq_use_env)
      apply (rule_tac diff_leq_use_env)
      apply (rule_tac r_sb="diff_use_env r_s2b (comp_use_env (ereq_use_env c tau_x) r_exa)" in trans_leq_use_env)
       apply (rule_tac diff_leq_use_env)
       apply (auto)
    apply (cut_tac env="env" and ?r_s1.0="r_s2a" and e="v" and ?r_s2.0="r_s3" and rx="rx2" in infl_sexp_wp)
      apply (auto)
...
lemma safe_send_exp: "\<lbrakk> well_typed env r_s1 (app_hole h (AppExp (AppExp (ConstExp SendConst) (VarExp x a)) v)) tau r_s2 rx;
  valid_nres_map s rs_map; valid_exp_use_env s rs_map r_s1; wf_hole h; is_value v; a \<noteq> NoRef \<rbrakk> \<Longrightarrow>
  ...
  apply (rule_tac well_typed_diff_perms)
   apply (rule_tac c="VarExp x a" in safe_send_exp_ih)
     apply (auto)
  ...
  apply (case_tac "xa \<in> non_prim_vars env v")
   apply (cut_tac h="h" and x="xa" and e="ConstExp UnitConst" in app_hole_free_vars_rev)
    apply (auto)
    apply (simp add: non_prim_vars_def)
   apply (cut_tac h="h" and x="xa" and env="env" and ?r_s1.0="r_s1" and v="VarExp x a" and e="v" in safe_send_hole_npv_use)
        apply (auto)
    (* otherwise, xa is in the completion. we identify its ancestor z. *)
  apply (simp add: own_env_vars_def)
  apply (simp add: full_dom_use_env_def)
  apply (simp add: dom_use_env_def)
  apply (case_tac "\<exists>l z. z \<in> non_prim_vars env v \<and> path_lookup rs_map z l xa")
   apply (auto)
    (* we note that z is a np-var of e therefore r_s1 z = Own *)
  apply (cut_tac x="z" and env="env" and f="AppExp (ConstExp SendConst) (VarExp x a)" in safe_own_npv_use)
        apply (auto)
   apply (simp add: pure_fun_def)
   apply (simp add: is_own_def)
  ...
  apply (case_tac "r_s1 xa \<noteq> NoPerm")
  ...
  apply (cut_tac env="env" and ?r_s1.0="r_s1" and x="xa" in well_typed_no_npv_use)
    apply (auto)
  apply (simp add: non_prim_vars_def)    
    (* the very last part is showing that x not in 'h (send c v)' implies x not in 'h ()'. *)
  apply (cut_tac x="xa" and h="h" and e="ConstExp UnitConst" in app_hole_free_vars_rev)
   apply (auto)
  apply (cut_tac x="xa" and h="h" and e="AppExp (AppExp (ConstExp SendConst) (VarExp x a)) v" in app_hole_free_vars2)
   apply (auto)
  done
...
lemma srps_send_case: "\<lbrakk>well_typed_system env rs_map p_map s2 ps1; valid_reduct app_red_exp; r_ax = SendAct;
        ps1 u1 = Some (app_hole h1 (AppExp (AppExp (ConstExp SendConst) (VarExp x_s a_s)) v));
        ps1 u2 = Some (app_hole h2 (AppExp (ConstExp RecvConst) (VarExp x_r a_r))); u1 \<noteq> u2; wf_hole h1;
        wf_hole h2; is_value v; a_s \<noteq> NoRef; a_r \<noteq> NoRef; s2 (deref_name x_s a_s) = Some ChanSValue;
        s2 (deref_name x_r a_r) = Some (ChanRValue (deref_name x_s a_s)); s1 = s2;
  ...
  apply (case_tac "\<not> (\<exists> t. env (deref_name x_s a_s) = Some (ChanTy t SEnd))")
   apply (simp add: well_typed_state_def)
   apply (auto)
   apply (erule_tac x="deref_name x_s a_s" in allE)
   apply (auto)
   apply (case_tac "env (deref_name x_s a_s)")
    apply (auto)
    (* and the recv channel's type *)
  apply (case_tac "\<not> env (deref_name x_r a_r) = Some (ChanTy t REnd)")
   apply (simp add: well_typed_state_def)
   apply (auto)
   apply (erule_tac x="deref_name x_r a_r" in allE)
   apply (auto)
   apply (case_tac "env (deref_name x_r a_r)")
    apply (auto)
    (* prelim: we prove the dominator of v \<le> lift a r (the permission map of u1) *)
  apply (cut_tac eq_own)
  apply (auto)
  apply (cut_tac r_sc="np_dom_use_env env v" and r_sb="np_dom_use_env env (app_hole h1 (AppExp (AppExp (ConstExp SendConst) (VarExp x_s a_s)) v))"
   ...
   apply (auto)
   apply (simp add: non_prim_vars_def)
   apply (rule_tac app_hole_free_vars)
   apply (auto)
  ...
  apply (cut_tac env="env" and v="v" and h="h1" and x="x_s" and a="a_s" and t="t" in safe_val_exp)
      apply (auto)
  ...
  apply (auto)
    ...
    apply (case_tac "u = u2")
     ...
     apply (rule_tac proper_app_hole)
      apply (simp)
     apply (cut_tac rs_map="rs_map" and h="h1" and e="AppExp (AppExp (ConstExp SendConst) (VarExp x_s a_s)) v" in proper_app_hole_split2)
      apply (simp)
     apply (simp add: proper_exp_def)
    apply (simp add: add_env_def)
